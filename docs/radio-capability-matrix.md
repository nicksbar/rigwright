...
| TX meters | `MeterId::{Power,Alc,Swr}` | H/P via `BG`/`SW` where queried; K4 typed unsolicited `TM` reports and `TM1`/`TM0` reporting toggle |
...
| TX status and additional meters | Power/ALC/SWR implemented; K4 `TM` event reports implemented; further status remains | Typed status/events, normalized meter fixtures, read-only semantics | RX/TX/tune captures for power/SWR/ALC/etc. |
