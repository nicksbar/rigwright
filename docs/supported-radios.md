...
For generated model status, baud choices, HAL surface, and evidence, see
[`generated-support-matrix.md`](generated-support-matrix.md). For the complete
control, meter, manual-surface, and QSONaut-consumption inventory, see
[`radio-capability-matrix.md`](radio-capability-matrix.md).

This document is the manual-source and citation index. Support labels belong to
the generated matrix, so model status is not duplicated here. **Hardware
validated** means the driver is exercised against a physical radio. **Framework**
means a model profile and protocol primitives exist, but Rigwright does not yet
claim a working end-to-end driver for that radio.
