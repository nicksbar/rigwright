...
impl SupportMatrix {
    ...
    /// Render the catalog projection for release notes and human review.
    ///
    /// The generated section is deliberately limited to facts owned by the
    /// catalog and profiles. Manual citations and narrative capability notes
    ...
    pub fn to_markdown(&self) -> String {
        ...
        output.push_str("# Generated radio support matrix\n\n");
        output.push_str("This file is generated from `POPULAR_RADIOS` and the selected vendor profiles. It distinguishes catalog/profile/software evidence from physical hardware evidence.\n\n");
        output.push_str("| Manufacturer | Model | Protocol | Support level | Baud rates | Preferred | Cataloged | Software-tested | Hardware-tested | HAL surface | Evidence |\n");
        output.push_str("|---|---|---|---|---|---:|:---:|:---:|:---:|---|---|\n");
        for entry in &self.models {
            let hal = format!(
                "freq {}/{}; mode {}/{}; PTT {}/{}; power {}/{}",
                yes_no(entry.hal.can_get_frequency),
                yes_no(entry.hal.can_set_frequency),
                yes_no(entry.hal.can_get_mode),
                yes_no(entry.hal.can_set_mode),
                yes_no(entry.hal.can_get_ptt),
                yes_no(entry.hal.can_set_ptt),
                yes_no(entry.hal.can_get_power),
                yes_no(entry.hal.can_set_power),
            );
            let baud_rates = entry
                .baud_rates
                .iter()
                .map(u32::to_string)
                .collect::<Vec<_>>()
                .join(", ");
            let references = if entry.evidence.references.is_empty() {
                "—".to_owned()
            } else {
                entry.evidence.references.join("; ")
            };
            output.push_str(&format!(
                ...
                entry.support_level,
                baud_rates,
                entry.preferred_baud_rate,
                yes_no(entry.evidence.cataloged),
                yes_no(entry.evidence.software_tested),
                yes_no(entry.evidence.hardware_tested),
                hal,
                references,
            ));
        }
...
mod tests {
    ...
    #[test]
    fn support_matrix_markdown_is_generated_and_contains_evidence_columns() {
        let markdown = SupportMatrix::from_catalog().to_markdown();
        assert!(markdown.starts_with("<!-- Generated by"));
        assert!(markdown.contains("| Manufacturer | Model | Protocol |"));
        assert!(markdown.contains("| Icom | IC-7300 | Icom CI-V |"));
        assert!(markdown.contains("| yes | yes | yes |"));
    }
}
